...
export async function GET(_req: Request, { params }: Ctx) {
  ...
  const infoMap: Record<string, { name: string; cpf: string | null }> = {}
  for (let i = 0; i < emails.length; i += 30) {
    ...
    for (const doc of snap.docs) {
      const data = doc.data()
      if (data.email) infoMap[data.email] = { name: data.name, cpf: data.cpf ?? null }
    }
  }

  return Response.json(emails.map((email) => ({
    email,
    name: infoMap[email]?.name ?? null,
    cpf: infoMap[email]?.cpf ?? null,
  })))
}
